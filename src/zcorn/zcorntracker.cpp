...
CzCORNTracker::CzCORNTracker(std::string strWalletFile)
{
...
CzCORNTracker::~CzCORNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCORNTracker::Init()
{
...
bool CzCORNTracker::Archive(CMintMeta& meta)
{
...
bool CzCORNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCORNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCORNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCORNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCORNTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCORNTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCORNTracker::GetSerialHashes()
{
...
CAmount CzCORNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCORNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCORNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCORNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCORNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCORNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCORNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCORNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCORNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCORNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCORNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCORNWallet* zCORNWallet)
{
    bool iszCORNWalletInitialized = (NULL != zCORNWallet);
    CMintMeta meta;
    ...
    if (! iszCORNWalletInitialized)
        zCORNWallet = new CzCORNWallet(strWalletFile);
    meta.isSeedCorrect = zCORNWallet->CheckSeed(dMint);
    if (! iszCORNWalletInitialized)
        delete zCORNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCORNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCORNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCORNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCORNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCORNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCORNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCORNWallet* zCORNWallet = new CzCORNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCORNWallet);
        }
        delete zCORNWallet;
        LogPrint("zero", "%s: added %d dzcorn from DB\n", __func__, listDeterministicDB.size());
    }
