...
CzCORNWallet::CzCORNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcorn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCORNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCORNSeed_deprecated()) {
                    LogPrintf("%s: Updated zCORN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcorn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcorn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCORNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCORNCount(nCountLastUsed);
    else if (!walletdb.ReadZCORNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCORNWallet::Lock()
{
    seedMaster = 0;
}

void CzCORNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCORNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCORN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCORNWallet::LoadMintPoolFromDB()
{
...
void CzCORNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCORNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCORNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcornTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCORNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCORN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcornTracker which also adds to database
    pwalletMain->zcornTracker->Add(dMint, true);
...
void CzCORNWallet::SeedToZCORN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCORNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCORNWallet::GenerateDeterministicZCORN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCORNWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCORNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCORNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
