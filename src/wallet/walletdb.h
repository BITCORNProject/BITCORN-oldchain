...
public:
    ...
    bool WriteZCORNSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZCORNSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZCORNSeed_deprecated(uint256& seed);
    bool EraseZCORNSeed();
    bool EraseZCORNSeed_deprecated();

    bool WriteZCORNCount(const uint32_t& nCount);
    bool ReadZCORNCount(uint32_t& nCount);
