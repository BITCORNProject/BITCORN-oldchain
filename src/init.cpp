...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, bitcorn, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-backupzbitcorn=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCORN minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizebitcornamount=<n>     " + strprintf(_("Keep N BITCORN anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The BitCorn Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitCorn Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitCorn Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZBitCornBackups = GetBoolArg("-backupzbitcorn", true);
        pwalletMain->setZBitCornAutoBackups(fEnableZBitCornBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeBitCornAmount = GetArg("-anonymizebitcornamount", 0);
//    if (nAnonymizeBitCornAmount > 999999) nAnonymizeBitCornAmount = 999999;
//    if (nAnonymizeBitCornAmount < 2) nAnonymizeBitCornAmount = 2;
    ...
    LogPrintf("Anonymize BitCorn Amount %d\n", nAnonymizeBitCornAmount);
