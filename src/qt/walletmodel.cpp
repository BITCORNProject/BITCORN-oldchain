...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bitcorn address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CORN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bitcorn:URI (bitcorn:XyZ...?message=example)
            {
...
bool WalletModel::createZcornSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZcorn(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
    ...
    if(!isMine(addressTo)){
        receipt.SetStatus(_("To convert zCORN back to CORN the return address must be from your wallet"), ZCORN_SPEND_ERROR);
        return false;
    }
...
static void NotifyzCORNReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCORNReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCORNReset.connect(boost::bind(NotifyzCORNReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCORNReset.disconnect(boost::bind(NotifyzCORNReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcornTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
