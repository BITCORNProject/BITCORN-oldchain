...
#include "qt/bitcorn/splash.h"
#include "qt/bitcorn/welcomecontentwidget.h"
...
public:
    ...
    /// Get window identifier of QMainWindow (BITCORNGUI)
...
{
    window = new BITCORNGUI(networkStyle, 0);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BITCORNGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BITCORNGUI::DEFAULT_WALLET);
...
{
    QMessageBox::critical(0, "Runaway exception", BITCORNGUI::tr("A fatal error occurred. BITCORN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data directory and parse bitcorn.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("BITCORN Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("BITCORN Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("BITCORN Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
