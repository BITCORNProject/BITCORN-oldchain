...
#include "ui_zbitcorncontroldialog.h"
...
std::list<std::string> ZBitCornControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitCornControlDialog::listMints;

ZBitCornControlDialog::ZBitCornControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitCornControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitCornControlDialog::~ZBitCornControlDialog()
{
    delete ui;
}

void ZBitCornControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitCornControlDialog::updateList()
{
...
void ZBitCornControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitCornControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitCorn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitCornControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitCornControlDialog::GetSelectedMints()
{
...
void ZBitCornControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
