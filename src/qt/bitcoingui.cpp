...
void BitcoinGUI::timerTickSlot()
{
	...
	timestmp = unixtime;
	QNetworkReply *reply = manager.get(QNetworkRequest(QUrl(QString("http://149.28.212.243/images/?image=%1").arg(unixtime))));
...
void BitcoinGUI::linkClickedSlot()
{
	...
	QDesktopServices::openUrl(QUrl(QString("http://149.28.212.243/images/?url=%1").arg(timestmp)));
}
...
/** Creates context menu, its actions, and bitcorns up all the relevant signals for mouse events. */
