...
#include "ui_zcorncontroldialog.h"
...
std::set<std::string> ZCornControlDialog::setSelectedMints;
std::set<CMintMeta> ZCornControlDialog::setMints;

bool CZCornControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZCornControlDialog::COLUMN_DENOMINATION || column == ZCornControlDialog::COLUMN_VERSION || column == ZCornControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZCornControlDialog::ZCornControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCornControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zCORN Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZCorn->setProperty("cssClass", "text-main-purple");
    ui->labelZCorn_int->setProperty("cssClass", "text-main-purple");
...
ZCornControlDialog::~ZCornControlDialog()
{
    delete ui;
}

void ZCornControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCornControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZCornControlWidgetItem* itemDenom(new CZCornControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZCornControlWidgetItem *itemMint = new CZCornControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zcornTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcornTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCORN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCORN seed used to mint this zCORN is not the same as currently hold in the wallet";
            else
...
void ZCornControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCornControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCorn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZCornControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCornControlDialog::GetSelectedMints()
{
...
void ZCornControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
