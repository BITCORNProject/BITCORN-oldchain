...
void MultisigDialog::on_importAddressButton_clicked() {
	...
	if (sRedeem.empty()) {
		ui->addMultisigStatus->setStyleSheet("QLabel { color: #e9a825; }");
...
void MultisigDialog::on_createButton_clicked()
{
	...
	try {
		...
		if (CoinControlDialog::coinControl->HasSelected()) {
			...
			for (int i = 0; i < ui->inputsList->count(); i++) {
				...
				if (txIdLine->text().isEmpty()) {
					ui->createButtonStatus->setStyleSheet("QLabel { color: #e9a825; }");
		...
		ui->createButtonStatus->setStyleSheet("QTextEdit{ color: #e9a825 }");
...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet, string& errorRet)
{
	try {
		...
		if (totalIn < totalOut) {
			throw runtime_error("Not enough BITCORN provided as input to complete transaction (including fee).");
		}
		...
		if (tx.vout.at(changeIndex).nValue > fee) {
			...
			throw runtime_error("Not enough BITCORN provided to cover fee");
		}
...
void MultisigDialog::on_signButton_clicked()
{
	...
	try {
		...
		ui->signButtonStatus->setStyleSheet("QTextEdit{ color: #e9a825 }");
...
void MultisigDialog::on_addAddressButton_clicked()
{
	...
	addressFrame->setStyleSheet("QFrame { background-color: #1b2032; }");
...
void MultisigDialog::on_addInputButton_clicked()
{
	...
	txInputFrame->setStyleSheet("QFrame { background-color: #1b2032; }");
...
void MultisigDialog::on_addDestinationButton_clicked()
{
	...
	destinationFrame->setStyleSheet("QFrame { background-color: #1b2032; }");
...
void MultisigDialog::on_addPrivKeyButton_clicked()
{
	...
	keyFrame->setStyleSheet("QFrame { background-color: #1b2032; }");
