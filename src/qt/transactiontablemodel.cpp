...
// Copyright (c) 2016-2019 The BITCORN developers
	...
	*/
	...
	void updateHasZcTxesIfNeeded(const TransactionRecord& record) {
		...
		if (record.type == TransactionRecord::ZerocoinMint ||
			...
			record.type == TransactionRecord::ZerocoinSpend_Change_zCorn ||
...
QString TransactionTableModel::formatTxType(const TransactionRecord* wtx) const
{
	...
	case TransactionRecord::StakeMint:
		return tr("CORN Stake");
	case TransactionRecord::StakeZCORN:
		return tr("zCORN Stake");
	...
	case TransactionRecord::ZerocoinMint:
		return tr("Converted CORN to zCORN");
	case TransactionRecord::ZerocoinSpend:
		return tr("Spent zCORN");
	case TransactionRecord::RecvFromZerocoinSpend:
		return tr("Received CORN from zCORN");
	case TransactionRecord::ZerocoinSpend_Change_zCorn:
		return tr("Minted Change as zCORN from zCORN Spend");
	case TransactionRecord::ZerocoinSpend_FromMe:
		return tr("Converted zCORN to CORN");
	default:
		return QString();
	}
}
...
QVariant TransactionTableModel::data(const QModelIndex& index, int role) const
{
	...
	case Qt::ForegroundRole:
		...
		if (rec->type == TransactionRecord::Generated || rec->type == TransactionRecord::StakeMint ||
			rec->type == TransactionRecord::StakeZCORN || rec->type == TransactionRecord::MNReward) {
