...
#include "qt/bitcorn/dashboardwidget.h"
#include "qt/bitcorn/forms/ui_dashboardwidget.h"
#include "qt/bitcorn/sendconfirmdialog.h"
#include "qt/bitcorn/txrow.h"
...
DashboardWidget::DashboardWidget(BITCORNGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	// Staking Information
	ui->labelMessage->setText(tr("Amount of CORN staked."));
	setCssSubtitleScreen(ui->labelMessage);
	setCssProperty(ui->labelSquareCorn, "square-chart-corn");
	setCssProperty(ui->labelCorn, "text-chart-corn");
	...
	ui->labelAmountCorn->setText("0 CORN");
	setCssProperty(ui->labelAmountCorn, "text-stake-corn-disable");
	...
	ui->btnHowTo->setText(tr("How to get CORN"));
...
void DashboardWidget::changeChartColors() {
	...
	if (isLightTheme()) {
		...
		gridLineColorX = QColor(13, 16, 26);
		linePenColorY = gridLineColorX;
		backgroundColor = linePenColorY;
	}
...
void DashboardWidget::loadChartData(bool withMonthNames) {
	...
	chartData->amountsByCache = getAmountBy(); // pair CORN
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			corn = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalCorn += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesCorn.append(corn);

		int max = std::max(corn, corn);
...
void DashboardWidget::onChartRefreshed() {
	...
	// init sets
	set0 = new QBarSet("CORN");

	set0->setColor(QColor(127, 110, 76));
	...
	set0->append(chartData->valuesCorn);


	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalCorn > 0) {
		setCssProperty(ui->labelAmountCorn, "text-stake-corn");
	}
	else {
		setCssProperty(ui->labelAmountCorn, "text-stake-corn-disable");
	}
	forceUpdateStyle({ ui->labelAmountCorn });
	ui->labelAmountCorn->setText(GUIUtil::formatBalance(chartData->totalCorn, nDisplayUnit));
