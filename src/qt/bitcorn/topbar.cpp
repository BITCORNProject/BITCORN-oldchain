...
#include "qt/bitcorn/topbar.h"
#include "qt/bitcorn/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/bitcorn/lockunlock.h"
#include "qt/bitcorn/qtutils.h"
#include "qt/bitcorn/receivedialog.h"
...
TopBar::TopBar(BITCORNGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty(ui->labelAmountTopCorn, "amount-small-topbar");
    setCssProperty(ui->labelAmountCorn, "amount-topbar");
    setCssProperty({ui->labelPendingCorn, ui->labelImmatureCorn}, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount cornAvailableBalance = balance - immatureBalance - nLockedBalance;



    // Set
    QString totalCorn = GUIUtil::formatBalance(cornAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopCorn->setText(totalCorn);

    // Expanded
    ui->labelAmountCorn->setText(totalCorn);

    ui->labelPendingCorn->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureCorn->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
