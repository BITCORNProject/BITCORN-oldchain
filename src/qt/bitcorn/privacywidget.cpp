...
#include "qt/bitcorn/privacywidget.h"
#include "qt/bitcorn/forms/ui_privacywidget.h"
#include "qt/bitcorn/qtutils.h"
#include "guiutil.h"
#include "qt/bitcorn/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(BITCORNGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zCORN anonymizes your CORN by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zCORN or convert back to CORN"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 CORN ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zCORN");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zCORN");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zCORN");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zCORN");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zCORN");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zCORN");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zCORN");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zCORN");
    ...
    // Only Convert to CORN enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzCORN->setTitleClassAndText("btn-title-grey", "Total 0 zCORN");
    ui->btnTotalzCORN->setSubTitleClassAndText("text-subtitle", "Show denominations of zCORN owned.");
    ui->btnTotalzCORN->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select CORN outputs to mint into zCORN.");
    ...
    connect(ui->btnTotalzCORN, SIGNAL(clicked()), this, SLOT(onTotalZcornClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zCORN");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of CORN to mint into zCORN"));
    }else{
        btnText = tr("Convert back to CORN");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zCORN to convert back into CORN"));
    }
...
void PrivacyWidget::onTotalZcornClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzCORN->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzCORN->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zCORN is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zCORN, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zCORN").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zCORN minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcorn(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zCORN converted back to CORN"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any CORN to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzCORN->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
