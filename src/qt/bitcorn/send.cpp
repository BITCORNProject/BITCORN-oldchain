...
#include "qt/bitcorn/send.h"
#include "qt/bitcorn/forms/ui_send.h"
#include "qt/bitcorn/addnewcontactdialog.h"
#include "qt/bitcorn/qtutils.h"
#include "qt/bitcorn/sendchangeaddressdialog.h"
#include "qt/bitcorn/optionbutton.h"
#include "qt/bitcorn/sendconfirmdialog.h"
#include "qt/bitcorn/myaddressrow.h"
...
SendWidget::SendWidget(BITCORNGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (CORN)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a BITCORN address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 CORN");
	...
	setCssProperty(coinIcon, "coin-icon-corn");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_CORN, true)) {
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress cornAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(cornAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(cornAdd.Get(), dialog->getLabel().toStdString(), "send")) {
