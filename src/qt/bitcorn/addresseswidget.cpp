...
#include "qt/bitcorn/forms/ui_addresseswidget.h"
#include "qt/bitcorn/addresslabelrow.h"
#include "qt/bitcorn/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(BITCORNGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a BITCORN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress cornAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(cornAdd.Get(), label.toUtf8().constData(), "send")) {
