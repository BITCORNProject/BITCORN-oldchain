...
#include "zbitcorncontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCORN ought to be enough for anybody." - Bill Gates, 2017
    ui->zCORNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCORNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCORN->setEnabled(false);
        ui->pushButtonMintzCORN->setToolTip(tr("zCORN is currently disabled due to maintenance."));

        ui->pushButtonSpendzCORN->setEnabled(false);
        ui->pushButtonSpendzCORN->setToolTip(tr("zCORN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCORNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCORN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCORN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCORN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCORN in ") +
...
void PrivacyDialog::on_pushButtonSpendzCORN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCORN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCORN
        sendzCORN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCORN
    sendzCORN();
}

void PrivacyDialog::on_pushButtonZBitCornControl_clicked()
{
    ZBitCornControlDialog* zCORNControl = new ZBitCornControlDialog(this);
    zCORNControl->setModel(walletModel);
    zCORNControl->exec();
}

void PrivacyDialog::setZBitCornControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCORNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCORN()
{
    ...
    double dAmount = ui->zCORNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCORN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BITCORN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCORN</b>";
    ...
    // use mints from zCORN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitCornControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitCornControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbitcorn selector in case it was used
    ZBitCornControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCORN Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCORN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCORN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCORN "));
    ui->labelzCORNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCORNSyncStatus->setVisible(fShow);
}
