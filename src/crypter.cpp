    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZCORNSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZCORNSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zcornseed to wallet";
    }
                //the use case for this is no password set seed, mint dzCORN,
...
bool CCryptoKeyStore::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZCORNSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZCORNSeed(hashSeed, vchSeed)) {
